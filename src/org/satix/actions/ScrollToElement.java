...
/**
 * This class defines the scrollToElement action which moves scrollBar on 
 * an element till another element is into view either by id, or xpath, or css selector.
 * 
 * @author Qi En Jiang
 * 
 */
public class ScrollToElement extends Action{
	private static final String CLASSNAME = ScrollToElement.class.getSimpleName();
	private static final Logger logger = SatixLogger.getLogger(CLASSNAME);
	private int interval = 1000;
	private String toElement;
	private String toElementBy;
	...
	public void setToElementBy(String toElementBy) {
		this.toElementBy = toElementBy;
	}

	public String getToElementBy() {
		return toElementBy;
	}
			...
			if (ElementBy.ID.getName().equals(getElementBy())) {
				...
				if (!browser.contains(Browsers.IE.getName())) {
					...
					throw new ActionException(this, "Action error: name \""	+ getName() + "\" number \"" + getNumber() 
							+ "\" line \""	+ getLine() + "\" column \"" + getColumn() + "\" xpath is not supported in scrollToElement action in IE");
				}
	        ...
	        while (tries > 0) {
	        	try {				
					...
					if (scrollTop.equals(scrollHeight)) {
						break;
					} else if (WaitForWebElementToLoad(driver, getToElement(),
							getToElementBy()).isDisplayed()) {
						break;
					}
